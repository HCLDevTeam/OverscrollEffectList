...
class CustomOverscrollEffect(val scope: CoroutineScope, val orientation: Orientation) : OverscrollEffect {
...
//        isVertical = abs(delta.y) > abs(delta.x)
        ...
        val consumedByPreScroll = if (abs(overscrollOffset.value) > 0.5 && !sameDirection) {
            ...
            if (sign(prevOverscrollValue) != sign(newOverscrollValue)) {
                ...
                if (orientation == Orientation.Vertical) {
                    Offset(x = 0f, y = scrollDelta + prevOverscrollValue) // we did not consume any offset
                } else {
        ...
        val overscrollToAdd = if(orientation == Orientation.Vertical) overscrollDelta.y else overscrollDelta.x
    ...
    override suspend fun applyToFling(
        ...
        val scrollVelocity = if(abs(velocity.y) > abs(velocity.x)) velocity.y else velocity.x
        ...
        object : Modifier.Node(), LayoutModifierNode {
            override fun MeasureScope.measure(
                ...
                return layout(placeable.width, placeable.height) {
                    val offsetValue = if (orientation == Orientation.Vertical) IntOffset(x = 0, y = overscrollOffset.value.roundToInt()) else
                        IntOffset(x = overscrollOffset.value.roundToInt(), y = 0)
