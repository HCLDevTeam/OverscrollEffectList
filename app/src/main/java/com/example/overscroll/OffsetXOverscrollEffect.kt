...
class OffsetXOverscrollEffect(val scope: CoroutineScope) : OverscrollEffect {
    ...
    override fun applyToScroll(
        ...
        val consumedByPreScroll =
            if (abs(overscrollOffset.value) > 0.5 && !sameDirection) {
                ...
                if (sign(prevOverscrollValue) != sign(newOverscrollValue)) {
                    ...
                    Offset(y = 0f, x = delta.x + prevOverscrollValue)
                } else {
        ...
        if (abs(overscrollDelta.x) > 0.5 && source.equals(MutatePriority.UserInput)) {
            scope.launch {
        ...
        object : Modifier.Node(), LayoutModifierNode {
            override fun MeasureScope.measure(
                ...
                return layout(placeable.width, placeable.height) {
                    val offsetValue = IntOffset(y = 0, x = overscrollOffset.value.roundToInt())
